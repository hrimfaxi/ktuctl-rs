...
- `config.rs` 中的协议结构体使用 `#[repr(C)]` 并带有显式填充字段（`_pad0`、`_pad1`、`reserved0` 等），必须与内核模块的 C 结构体字节级对齐。使用 `zerocopy::{FromBytes, IntoBytes, Immutable}`。**不要重排或删除填充字段**——它们必须与内核模块的 C 结构体字节级对齐。
- `In6Addr` 使用 `#[repr(C, align(4))]` 包装 `[u8; 16]`，因为内核 `struct in6_addr` 含 `__u32` 成员，对齐为 4。如果用 `[u8; 16]`（对齐 1），所有包含地址的结构体都会差 2 字节。
...
- XOR 混淆支持：`client-add` 和 `server-add` 命令支持 `xor <hex_key>` 参数（1-64 字节十六进制密钥）。密钥存储在 egress/ingress/user_info 结构体的 `xor_key` + `xor_key_len` 字段中。
